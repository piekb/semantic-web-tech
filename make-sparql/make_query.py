...
# Settings
input_folder = "../data-rewrite/output/"
output_folder = "result/"
base_names = ["train-", "test-"]
db = "<http://dbpedia.org/"
...
# Should find the correct term to use somehow. Currently random
...
def make_terms(l):
    new_l = []
    for w in l.split("|"):
        if w in hardcode_words:
            new_l.append(hardcode_words[w])
        else:
            skip = False
            bracket = False
            x = ""
            for i, c in enumerate(w):
                if c == " " and i+1 < len(w):
                    if w[i + 1].isupper() or w[i + 1] == "(" or bracket:
                        x += "_"
                        if w[i + 1] == "(":
                            bracket = True
                    else:
...
# TODO: update with correct location of input data
for n in base_names:

    # Open file with encoded queries
    f_queries = open(input_folder + n + "queries.txt", "r")
    queries = f_queries.read().splitlines()  # to avoid \n being seen as part of last term

    # Open file with correct queries
    f_correct_queries = open(input_folder + n + "correct-sparql.txt", "r")
    correct_queries = f_correct_queries.read().splitlines()  # to avoid \n being seen as part of last term

    # Open file with named entities
    f_entities = open(input_folder + n + "named-entities.txt", "r")
    list_entities = f_entities.read().splitlines()  # to avoid \n being seen as part of last term

    # Files to write something to
    f_sparql_queries = open(output_folder + n + "sparql-queries.txt", "a")

    for j, query in enumerate(queries):
        # Make URI terms out of named entities, and compare to correct terms
